---
title: "2020-02-04: Epidemic Mitigation Time Forecast (Covid-19)"
date: 2020-02-04
...
ncovChina <- ncovChina[ncovChina[, "日期"] < as.Date('2020-02-04'), ]
```
...
if(as.Date('2020-02-04') < as.Date('2020-02-12')) {
  predict_date(province = "中国",  ncov = ncovChina, predDate = as.Date('2020-02-04'))
} else{

#nCov prediction, new model
#Yi Zou
#Last modified: "2020-02-12 21:04:50 CST"
PredDate <- format(as.Date('2020-02-04'),"%m-%d") #*****Set the date you want to predict

#Libarary and data
library(mgcv)
DtChina_all <- ncovChina[ncovChina[, "日期"] <= as.Date('2020-02-04'), ]
...
```{r, results='asis'}
if(as.Date('2020-02-04') < as.Date('2020-02-13')){
...
ncov$area <- ncov$area[ncov$area$date < as.Date('2020-02-04'), ]
...
```{r, results='asis'}
cat("## 2020-02-04 Epidemic forecast by province")
```
...
for(x in Prov) {
  ...
  predict_date(province = x, ncov = ncov, ifplot = TRUE, ifchinese = FALSE, predDate = as.Date('2020-02-04'))
  cat("  \n")
}
```
