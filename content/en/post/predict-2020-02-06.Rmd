---
title: "2020-02-06: Epidemic Mitigation Time Forecast (Covid-19)"
date: 2020-02-06
slug: predict-2020-02-06
---
...
ncovChina <- ncovChina[ncovChina[, "日期"] < as.Date('2020-02-06'), ]
```
...
if(as.Date('2020-02-06') < as.Date('2020-02-12')) {
  predict_date(province = "中国",  ncov = ncovChina, predDate = as.Date('2020-02-06'))
} else{

#nCov prediction, new model
#Yi Zou
#Last modified: "2020-02-12 21:04:50 CST"
PredDate <- format(as.Date('2020-02-06'),"%m-%d") #*****Set the date you want to predict

#Libarary and data
library(mgcv)
DtChina_all <- ncovChina[ncovChina[, "日期"] <= as.Date('2020-02-06'), ]
...
Dat$Ymod <- Dat$Confirmed #Model varialbe, may change to "Total"
...
#Legend
legend("topleft", legend=c("Total infected","Daily increase","Total recover","Daily recover","Active infected"), col=c("red", "blue","green","purple","orange"), pch = 1, cex=0.6, bty='n')
...
```{r, results='asis'}
if(as.Date('2020-02-06') < as.Date('2020-02-13')){
  cat("Our model shows that the national epidemic situation will ease on", pre$enddate, "(the number of confirmed cases will not increase significantly).  \\n\\n On", 
      pre$tomorrow,  "there will be", format(round(pre$tomorrowNew), scientific = FALSE), "new confirmed cases and in total", format(round(pre$tomorrowcount), scientific = FALSE), "confirmed cases."
  )
} else {
...
```{r, results='asis'}
cat("## 2020-02-06 Epidemic forecast by province")
```
...
for(x in Prov) {
  ...
  predict_date(province = x, ncov = ncov, ifplot = TRUE, ifchinese = FALSE, predDate = as.Date('2020-02-06'))
  cat("  \n")
}
```
