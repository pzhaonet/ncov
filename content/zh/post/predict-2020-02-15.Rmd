---
title: "2020-02-15: 疫情缓和时间预测 (Covid-19)"
date: 2020-02-15
...
```{r}
pre <- predict_date(province = "中国",  ncov = ncovChina, ifplot = FALSE, PredDate =  as.Date('2020-02-15'))
...
df_predict <- t(sapply(unique(ncov$area$provinceShortName[ncov$area$country == '中国']), function(x) predict_date(province = x, ncov = ncov, ifplot = FALSE, PredDate = as.Date(  '2020-02-15'))))
...
for(x in Prov) {
  cat(x, ":  \n")
    predict_date(province = x,  ncov = ncov, PredDate = as.Date('2020-02-15'))
  cat("  \n")
}
```
