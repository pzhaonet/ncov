---
title: "2020-02-28: 疫情缓和时间预测 (Covid-19)"
date: 2020-02-28
slug: predict-2020-02-28
---
...
ncovChina <- get_ncov('china') 
ncovChina <- ncovChina[ncovChina[, "日期"] < as.Date('2020-02-28'), ]
```
...
if(as.Date('2020-02-28') < as.Date('2020-02-13')) {
  predict_date(province = "中国",  ncov = ncovChina, PredDate = as.Date('2020-02-28'))
} else{

#nCov prediction, new model
#Yi Zou
#Last modified: "2020-02-26 14:33:59 CST"
PredDate <- format(as.Date('2020-02-28') ,"%m-%d") #*****Set the date you want to predict

#Libarary and data
library(mgcv)
DtChina_all <- ncovChina[ncovChina[, "日期"] < as.Date('2020-02-28'), ]
...
#-----------Another way to calculate current sick 2020-02-26
     ...
     fitdc <- nls(RateC~SSlogis(Day, Asym, xmid, scal),data= Dat[-c(1:8,24:26),] )
     ac <-  summary(fitdc)$coefficients[1,1]
     bc <-  summary(fitdc)$coefficients[2,1]
     cc <-  summary(fitdc)$coefficients[3,1]
     
     Fun <- (1+Mrd+ac/(1+exp((bc-Pred)/cc)))#Fixed daily death rate
    ...
    pSick[i] <- (newdat$pDaily[i]+pSick[i-1])/Fun[i]
     ...
     newdat$pSick <- pSick
     newdat$pDailyCured <- newdat$pSick*ac/(1+exp((bc-Pred)/cc))
...
```{r}
pre <- predict_date(province = "中国",  ncov = ncovChina, ifplot = FALSE, PredDate =  as.Date('2020-02-28'))
...
ncov$area <- ncov$area[ncov$area$date < as.Date('2020-02-28'), ]
df_predict <- t(sapply(unique(ncov$area$provinceShortName[ncov$area$countryName == '中国']), function(x) predict_date(province = x, ncov = ncov, ifplot = FALSE, PredDate = as.Date(  '2020-02-28'))))
...
for(x in Prov) {
  cat(x, ":  \n")
    predict_date(province = x,  ncov = ncov, PredDate = as.Date('2020-02-28'))
  cat("  \n")
}
```
