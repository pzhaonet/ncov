---
title: "2020-02-25: 疫情缓和时间预测 (Covid-19)"
date: 2020-02-25
slug: predict-2020-02-25
---
...
ncovChina <- get_ncov('china') 
ncovChina <- ncovChina[ncovChina[, "日期"] < as.Date('2020-02-25'), ]
```
...
if(as.Date('2020-02-25') < as.Date('2020-02-13')) {
  predict_date(province = "中国",  ncov = ncovChina, PredDate = as.Date('2020-02-25'))
} else{

#nCov prediction, new model
#Yi Zou
#Last modified: "2020-02-13 17:37:03 CST"
PredDate <- format(as.Date('2020-02-25') ,"%m-%d") #*****Set the date you want to predict

#Libarary and data
library(mgcv)
DtChina_all <- ncovChina[ncovChina[, "日期"] < as.Date('2020-02-25'), ]
...
```{r}
pre <- predict_date(province = "中国",  ncov = ncovChina, ifplot = FALSE, PredDate =  as.Date('2020-02-25'))
...
ncov$area <- ncov$area[ncov$area$date < as.Date('2020-02-25'), ]
df_predict <- t(sapply(unique(ncov$area$provinceShortName[ncov$area$countryName == '中国']), function(x) predict_date(province = x, ncov = ncov, ifplot = FALSE, PredDate = as.Date(  '2020-02-25'))))
...
for(x in Prov) {
  cat(x, ":  \n")
    predict_date(province = x,  ncov = ncov, PredDate = as.Date('2020-02-25'))
  cat("  \n")
}
```
