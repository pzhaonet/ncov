---
title: "2020-02-01: 疫情缓和时间预测 (2019-nCoV)"
date: 2020-02-01
...
pre <- predict_date(province = "中国",  ncov = ncovChina, ifplot = FALSE, PredDate = as.Date(  '2020-02-01'))
```
...
df_predict <- t(sapply(unique(ncov$area$provinceShortName[ncov$area$country == '中国']), function(x) predict_date(province = x, ncov = ncov, ifplot = FALSE, PredDate = as.Date(  '2020-02-01'))))
...
for(x in Prov) {
  cat(x, ":  \n")
    predict_date(province = x,  ncov = ncov, PredDate = as.Date(  '2020-02-01'))
  cat("  \n")
}
```
