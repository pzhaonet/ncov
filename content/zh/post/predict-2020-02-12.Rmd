---
title: "2020-02-12: 疫情缓和时间预测 (Covid-19)"
date: 2020-02-12
...
```{r}
pre <- predict_date(province = "中国",  ncov = ncovChina, ifplot = FALSE, PredDate =  as.Date('2020-02-12'))
...
if(as.Date('2020-02-12') < as.Date('2020-02-13')){
  cat("我们的模型显示，全国总体疫情将于", pre$enddate, "缓解（确诊病例不再明显增长）。  \\n\\n", 
      pre$tomorrow,  "确诊病例将新增", format(round(pre$tomorrowNew), scientific = FALSE), "例，累计确诊病例将达到", format(round(pre$tomorrowcount), scientific = FALSE), "例。"
  )
} else {
  cat("由于部分地区增加了临床诊断作为确诊依据，自2月13日起，我们的模型将采用“确诊+疑似”病例代替原来“确诊”病例作为我们的变量。")
}
```
...
df_predict <- t(sapply(unique(ncov$area$provinceShortName[ncov$area$country == '中国']), function(x) predict_date(province = x, ncov = ncov, ifplot = FALSE, PredDate = as.Date(  '2020-02-12'))))
...
for(x in Prov) {
  cat(x, ":  \n")
    predict_date(province = x,  ncov = ncov, PredDate = as.Date('2020-02-12'))
  cat("  \n")
}
```
