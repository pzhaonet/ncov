---
title: 2019-nCoV 疫情缓和时间预测(2020-02-03)
date: 2020-02-03
 ...
 #         geom_point(aes(y=New,x=Date,col="日增"),shape=1)+
  #        scale_colour_manual(name = '', values =c('blue','red'))
      ...
      p <- p+theme_bw(base_family = "STKaiti")+
          ...
          scale_x_date(date_breaks = "1 day", date_labels = "%m-%d",limits=c(as.Date("1-17","%m-%d"),Sys.Date()+20),expand=c(0,0.5))
      print(p)
#      return(p)
      } else {
        list(area = province,
...
```{r, results='asis'}
Prov <- c("湖北", "湖北以外", unique(ncov$area$provinceShortName[ncov$area$country == '中国' & ncov$area$provinceShortName != '湖北']))

for(x in Prov) {
  cat(x, ":  \n")
    predict_date(province = x,  ncov = ncov)
  cat("  \n")
}
```
